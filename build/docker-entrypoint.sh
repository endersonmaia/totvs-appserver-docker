...
if [ "$1" = 'appserver' ]; then
  ...
  /bin/sed 's/{{DBACCESS_SERVER}}/'"${DBACCESS_SERVER}"'/'      -i /totvs11/microsiga/protheus/bin/appserver/appserver.ini
  /bin/sed 's/{{DBACCESS_ALIAS}}/'"${DBACCESS_ALIAS}"'/'        -i /totvs11/microsiga/protheus/bin/appserver/appserver.ini
  /bin/sed 's/{{DBACCESS_PORT}}/'"${DBACCESS_PORT}"'/'        -i /totvs11/microsiga/protheus/bin/appserver/appserver.ini
